...
#define PANIC() do { debug_log("panic: exceptional condition; halting..."); exit(1); } while ( 1 ); 
...
#define EXPECTED_BLOCK_COUNT (CEILING(int(SAMPLE_COUNT)*CHANNEL_COUNT, 512))
...
#define BLOCK_BUFFER_COUNT ( CEILING(int(EXPECTED_BLOCK_COUNT), 4) )

// The number of samples in a block
#define BLOCK_SAMPLE_COUNT (SD_BUFFER_SIZE / (sizeof(int)*CHANNEL_COUNT))
...
CircularBuffer<block_t*, BLOCK_BUFFER_COUNT> g_ready_block;
...
//integer to hold the total number of writes complete. Will be used to end the reconrding once suffcient write occur.
int g_write_count=0;
...
void setup()
{
    ...
    bool cardPresent = g_sd_controller.begin((SdioConfig(FIFO_SDIO)));
	...
	do {
		...
		data_file_idx += 1;
		format_result = snprintf( filename,FILE_NAME_SIZE, FILENAME_FORMAT, data_file_idx);

		// Check if we have run out of data files
		if( format_result >= FILE_NAME_SIZE ) {
...
void sample_all_channels()
{
	...
	if ( g_current_sample >= BLOCK_SAMPLE_COUNT ) {
		...
		g_overrun_flag = ! g_ready_block.push(&g_blocks[g_current_block]);
		g_current_block += 1;
		g_current_sample = 0;
...
void loop()
{
	...
	if( g_ready_block.size() > 0 ){
		// Grab the next block pointer
		pBlock = g_ready_block.shift();
